...
ll ans = LLONG_MAX;
void try_row(const row &r, vector<int> &curr, int ind, ll cost) {
  if (curr == r.a) {
    ans = min(ans, cost);
    return;
  }
  ...
  try_row(r, curr, ind + 1, cost);
  vector<int> button = r.b[ind];
  for (int i = 0; i < button.size(); ++i) {
    curr[button[i]] ^= 1;
  }
  try_row(r, curr, ind + 1, cost + 1);
  for (int i = 0; i < button.size(); ++i) {
    curr[button[i]] ^= 1;
  }
}
...
void solve(void) {
  ...
  while (getline(cin, line)) {
    ...
    r.c = rc;
    rows.push_back(r);
    vector<int> currvec(ra.size());
    try_row(r, currvec, 0, 0);
    res += ans;
    ans = LLONG_MAX;
  }
  cout << res << endl;
